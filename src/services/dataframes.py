...
def get_quotes_dataframe(
    ...
    if not quotes_df.empty and suppliers_list:
        ...
        quotes_df["supplier_name"] = (
            quotes_df["supplier_id"].map(supplier_map).fillna("Fornecedor Desconhecido")
        )
    ...
    if not quotes_df.empty and items_list:
        ...
        quotes_df["item_name"] = (
            quotes_df["item_id"].map(item_map).fillna("Item Desconhecido")
        )
    elif not quotes_df.empty:
    ...
    if "created_at" in quotes_df.columns:
        quotes_df["created_at"] = pd.to_datetime(
            quotes_df["created_at"], errors="coerce"
        ).dt.tz_localize(None)
    if "updated_at" in quotes_df.columns:
        quotes_df["updated_at"] = pd.to_datetime(
            quotes_df["updated_at"], errors="coerce"
        ).dt.tz_localize(None)
    ...
    for col_name in final_columns:
        if col_name not in quotes_df.columns:
            if col_name in [
                ...
                quotes_df[col_name] = Decimal(
                    "0.0"
                )  # Or pd.NA if preferred for non-calculated numeric
            else:
...
def get_bids_dataframe(
    ...
    if not bids_df.empty and bidders_list:
        ...
        bids_df["bidder_name"] = bids_df.apply(
            lambda row: "Licitante Desconhecido"
            if pd.isna(row["bidder_id"])
            else bidder_map.get(row["bidder_id"], "Licitante Desconhecido"),
            axis=1,
        )
    ...
    if not bids_df.empty and items_list:
        item_map = {i.id: i.name for i in items_list}
        bids_df["item_name"] = (
            bids_df["item_id"].map(item_map).fillna("Item Desconhecido")
        )
    elif not bids_df.empty:
    ...
    if "created_at" in bids_df.columns:
        bids_df["created_at"] = pd.to_datetime(
            bids_df["created_at"], errors="coerce"
        ).dt.tz_localize(None)
    if "updated_at" in bids_df.columns:
        bids_df["updated_at"] = pd.to_datetime(
            bids_df["updated_at"], errors="coerce"
        ).dt.tz_localize(None)

    # Ensure 'price' is Decimal
    if "price" in bids_df.columns:
        bids_df["price"] = bids_df["price"].apply(
            lambda x: Decimal(str(x)) if x is not None else Decimal("0.0")
        )
    else:
