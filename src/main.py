...
# --- Inicialização do Session State (Mantém apenas flags de UI e IDs selecionados) ---
...
def load_suppliers_from_db() -> pd.DataFrame:
    """Carrega todos os fornecedores do banco de dados."""
    # TODO: Implementar a query para buscar fornecedores.
    return pd.DataFrame(
        columns=[
            "id",
            "name",
...
def load_competitors_from_db() -> pd.DataFrame:
    """Carrega todos os concorrentes do banco de dados."""
    # TODO: Implementar a query para buscar concorrentes.
    return pd.DataFrame(
        columns=[
            "id",
            "name",
...
def create_bids_figure(
    ...
    if (
        "timestamp" in bids_df_display.columns
        and not bids_df_display["timestamp"].isnull().all()
    ):
        b_df_sorted = (
            bids_df_display.sort_values(by="timestamp")
...
def _manage_generic_dialog(
    ...
    if entity_type == "bidding":
        ...
        db_entities_df = load_competitors_from_db()
    ...
    if st.session_state[editing_id_key] is not None:
        try:
            ...
            for field in form_fields_config.keys():
                if field in entity_to_edit_series:
                    data[field] = entity_to_edit_series[field]
            data["id"] = entity_to_edit_series["id"]
            dialog_mode = "edit"
        except IndexError:
            st.error(
                f"{title_singular} ID {st.session_state[editing_id_key]} não encontrado(a) para edição."
            )
    ...
    st.subheader(
        f"{'Editar' if dialog_mode == 'edit' else f'Novo(a)'} {title_singular}"
        + (
            f" (ID: {st.session_state[editing_id_key]})"
            if dialog_mode == "edit"
            else ""
        )
    )
    ...
    with st.form(key=f"{entity_type}_form"):
        ...
        for field, config in form_fields_config.items():
            ...
            if config["type"] == "text_input":
                ...
                form_data_submitted[field] = st.selectbox(
                    field_label, options=options, index=index
                )
                ...
                if pd.isna(val) or val is None:
                    ...
                    try:
                        ...
                        val = None
                form_data_submitted[field] = st.time_input(
                    field_label, value=val if isinstance(val, time) else None
                )
        ...
        if submitted:
            ...
            if not is_valid:
                ...
                save_data = {
                    k: v
                    for k, v in form_data_submitted.items()
                    if k in form_fields_config
                }
                ...
                if dialog_mode == "new":
                    ...
                    st.success(
                        f"{title_singular} '{updated_entity.get('name', updated_entity.get('process_number', ''))}' atualizado(a) com sucesso!"
                    )
    ...
    if st.session_state.get(confirm_delete_key, False):
        ...
        if related_entities_to_delete:
            warning_message += f" Todas as {', '.join(related_entities_to_delete)} associadas também serão deletadas. Esta ação não pode ser desfeita."
...
def manage_bidding_dialog_wrapper():
    _manage_generic_dialog(
        "bidding",
        form_fields_config=bidding_form_config,
        title_singular="Licitação",
        related_entities_to_delete=[
            "itens",
            "orçamentos dos itens",
            "lances dos itens",
        ],
    )
...
def manage_item_dialog_wrapper():
    ...
    try:
        ...
        st.info(
            f"Para Licitação: {parent_bidding_info['process_number']} - {parent_bidding_info['city']}"
        )
    except IndexError:
    ...
    _manage_generic_dialog(
        ...
        title_singular="Item",
        related_entities_to_delete=["orçamentos", "lances"],
...
def manage_supplier_dialog_wrapper():
    _manage_generic_dialog(
        "supplier",
        form_fields_config=contact_entity_form_config,
        title_singular="Fornecedor",
        related_entities_to_delete=["orçamentos"],
    )
...
def manage_competitor_dialog_wrapper():
    _manage_generic_dialog(
        "competitor",
        form_fields_config=contact_entity_form_config,
        title_singular="Concorrente",
        related_entities_to_delete=["lances"],
    )
...
if st.session_state.selected_item_id is not None:
    try:
        if not items_df_for_select.empty:
            current_item_details_series = items_df_for_select[
                items_df_for_select["id"] == st.session_state.selected_item_id
            ]
            if not current_item_details_series.empty:
                        ...
                        with st.form(key="new_quote_form"):
                            ...
                            if st.form_submit_button("💾 Salvar Orçamento"):
                                if selected_supplier_id_quote and quote_price > 0:
                                    ...
                                    st.error(
                                        "Selecione um fornecedor e insira um preço válido."
                                    )
                with expander_cols[1]:
                    with st.expander(
                        ...
                        competitor_options_map, competitor_option_ids = get_options_map(
                            df_input=competitors_df,
                            default_message=DEFAULT_COMPETITOR_SELECT_MESSAGE,
                        )
                        with col_comp_select:
                            selected_competitor_id_bid = st.selectbox(
                                "Concorrente*:",
                                options=competitor_option_ids,
                                format_func=lambda x: competitor_options_map.get(
                        ...
                        with col_comp_manage:
                            if st.button(
                                "👤 Ger. Concorrentes",
                                key="btn_manage_competitors_bid_exp",
                                use_container_width=True,
                            ):
                                st.session_state.editing_competitor_id = (
                                    selected_competitor_id_bid
                                )
                        ...
                        with st.form(key="new_bid_form"):
                            ...
                            if st.form_submit_button("💾 Salvar Lance"):
                                if selected_competitor_id_bid and bid_price > 0:
                                    ...
                                    st.error(
                                        "Selecione um concorrente e insira um preço válido."
                                    )

                quotes_for_item_df_display = quotes_df[
                    quotes_df["item_id"] == st.session_state.selected_item_id
                ].copy()
                ...
                with table_cols_display[0]:
                    ...
                    if not quotes_for_item_df_display.empty:
                        st.dataframe(
                            quotes_for_item_df_display[
                                ["supplier_name", "price", "notes", "updated_at"]
                            ],
                            hide_index=True,
                            use_container_width=True,
                        )
                ...
                with table_cols_display[1]:
                    ...
                    if not bids_for_item_df_display.empty:
                        ...
                        if "timestamp" in bids_to_show.columns:
                            bids_to_show["timestamp"] = pd.to_datetime(
                                bids_to_show["timestamp"]
