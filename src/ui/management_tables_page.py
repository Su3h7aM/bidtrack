...
def show_management_tables_view(bidding_repo, item_repo, supplier_repo, quote_repo, bidder_repo, bid_repo):
    ...
    with tab_items:
        ...
        # Use a unique key for the selectbox in this tab
        selected_bidding_id_for_items = st.selectbox(
            "Escolha uma Licitação:",
            options=bidding_option_ids,
            format_func=lambda x: bidding_options_map.get(x, "Selecione uma Licitação..."),
        ...
        df_items["updated_at"] = pd.to_datetime(df_items["updated_at"])
        ...
        for col in cols_to_display_items:
            if col not in df_items.columns:
                 df_items[col] = None # Add missing column with None
        ...
        column_config_items = {
            "id": st.column_config.NumberColumn("ID", disabled=True),
            "name": st.column_config.TextColumn("Nome do Item", required=True), # Changed label
            "description": st.column_config.TextColumn("Descrição do Item"), # Changed label
            "code": st.column_config.TextColumn("Código do Item"), # Changed label
            "quantity": st.column_config.NumberColumn("Quantidade", format="%.2f", min_value=0.01, required=True), # Refined
            "unit": st.column_config.TextColumn("Unidade", required=True),
            "bidding_id": st.column_config.NumberColumn("ID da Licitação Associada", disabled=True), # Changed label
            "created_at": st.column_config.DatetimeColumn("Criado em", format="YYYY-MM-DD HH:mm", disabled=True),
            "updated_at": st.column_config.DatetimeColumn("Atualizado em", format="YYYY-MM-DD HH:mm", disabled=True),
        }
        ...
        edited_items_df = st.data_editor(
            filtered_df_items,
            key="editor_itens_tab",
            column_config=column_config_items,
            num_rows="dynamic", # Again, add/delete would need more logic
            use_container_width=True
        )

        if st.button("Salvar Alterações em Itens", key="save_itens_tab"):
            item_changes_found = False
            original_items_subset = filtered_df_items.set_index('id')
            edited_items_subset = edited_items_df.set_index('id')

            for item_id, edited_item_row in edited_items_subset.iterrows():
                if item_id not in original_items_subset.index:
                    # New row logic - not covered by this update focused task
                    continue 
                
                original_item_row = original_items_subset.loc[item_id]
                item_update_dict = {}

                for col_name_item in edited_items_subset.columns:
                    original_item_value = original_item_row[col_name_item]
                    edited_item_value = edited_item_row[col_name_item]

                    if pd.isna(original_item_value) and pd.isna(edited_item_value):
                        continue
                    if isinstance(original_item_value, (pd.Timestamp, pd.NaTType)) and isinstance(edited_item_value, (pd.Timestamp, pd.NaTType)):
                        if (pd.isna(original_item_value) and pd.isna(edited_item_value)) or (original_item_value == edited_item_value):
                            continue
                    elif str(original_item_value) == str(edited_item_value):
                        continue
                    
                    item_update_dict[col_name_item] = edited_item_value
                
                if item_update_dict:
                    item_changes_found = True
                    try:
                        # Remove fields that should not be updated
                        item_update_dict.pop("id", None)
                        item_update_dict.pop("bidding_id", None)
            ...
            if item_changes_found:
                ...
                st.info("Nenhuma alteração detectada para salvar em Itens.")
    ...
    with tab_quotes:
        ...
        df_quotes_full = get_quotes_dataframe(quotes_list, suppliers_list) # This df has calculated_price

        # Add item_name to df_quotes_full
        item_names_map = {item.id: item.name for item in items_list}
        # The original quotes_list has item_id, df_quotes_full might need it added if not present
        # For simplicity, we can create a temporary series for item_id from quotes_list to map item_name
        # Or, if get_quotes_dataframe doesn't include item_id, we add it from quotes_list before mapping
        
        # Ensure 'id' from quote is the main id for the table, and add item_id if not present from get_quotes_dataframe
        # get_quotes_dataframe might rename quote.id to quote_id. We need to ensure we have the original quote.id
