...
def display_suppliers_tab(supplier_repo): # Add type hint: supplier_repo: SupplierRepository
    ...
    supplier_column_config = {
        ...
        "website": st.column_config.TextColumn("Website", help="Website do fornecedor (opcional)."),
        "email": st.column_config.TextColumn("Email", help="Email de contato do fornecedor (opcional)."),
        "phone": st.column_config.TextColumn("Telefone", help="Telefone de contato (opcional)."),
        "desc": st.column_config.TextColumn("Descrição", help="Descrição ou observações sobre o fornecedor (opcional)."),
        "created_at": st.column_config.DatetimeColumn("Criado em", format="YYYY-MM-DD HH:mm", disabled=True, help="Data de criação do registro."),
        "updated_at": st.column_config.DatetimeColumn("Atualizado em", format="YYYY-MM-DD HH:mm", disabled=True, help="Data da última atualização do registro."),
    }
