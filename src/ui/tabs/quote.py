...
from decimal import (
    Decimal,
)  # Keep for column config if needed, though calc is now in service
...
def prepare_quotes_dataframe_via_service(
    ...
    if (
        items_df.empty
        or "bidding_id" not in items_df.columns
        or "id" not in items_df.columns
    ):
        return pd.DataFrame()
    ...
    filtered_quotes_models = [
        quote
        for quote in all_quotes_list_models
        if quote.item_id in item_ids_for_selected_bidding
    ]
...
def display_quotes_tab(quote_repo, bidding_repo, item_repo, supplier_repo):
    ...
    quote_column_config = {
        ...
        "price": st.column_config.NumberColumn(
            ...
            format="R$ %.2f",
            required=True,
            help="Custo base do produto/serviço junto ao fornecedor.",
        ),
        "freight": st.column_config.NumberColumn(
            "Frete (R$)", format="R$ %.2f", help="Custo do frete (opcional)."
        ),
        "additional_costs": st.column_config.NumberColumn(
            "Custos Adic. (R$)",
            format="R$ %.2f",
            help="Outros custos adicionais (opcional).",
        ),
        "taxes": st.column_config.NumberColumn(
            "Impostos (%)",
            format="%.2f",
            help="Percentual de impostos sobre o preço + frete + custos (ex: 6 para 6%).",
        ),
        "margin": st.column_config.NumberColumn(
            "Margem (%)",
            format="%.2f",
            required=True,
            help="Margem de lucro desejada sobre o preço com impostos (ex: 20 para 20%).",
        ),
        "calculated_price": st.column_config.NumberColumn(
            "Preço Final Calculado",
            format="R$ %.2f",
            disabled=True,
            help="Preço final do orçamento para o cliente.",
        ),
        "notes": st.column_config.TextColumn(
            "Notas", help="Observações adicionais sobre o orçamento (opcional)."
        ),
        "link": st.column_config.LinkColumn(
            "Link do Produto",
            help="Link para a página do produto no site do fornecedor (opcional).",
            validate=r"^https?://[\w\.-]+",
        ),
        "item_id": st.column_config.NumberColumn("ID Item (Ref)", disabled=True),
        "supplier_id": st.column_config.NumberColumn(
            "ID Fornecedor (Ref)", disabled=True
        ),
    ...
    display_entity_management_ui(
        ...
        decimal_fields=[
            ...
            "taxes",
            "margin",
        ],  # Still needed for handle_save_changes
        fields_to_remove_before_update=[
            "item_name",
            "supplier_name",
            "calculated_price",
        ...
        fks_dict: prepare_quotes_dataframe_via_service(
            main_repo, fks_dict, item_repo, supplier_repo
        ),
