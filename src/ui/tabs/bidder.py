...
def display_bidders_tab(bidder_repo):  # bidder_repo: BidderRepository
    ...
    bidder_column_config = {
        "id": st.column_config.NumberColumn(
            "ID", disabled=True, help="ID único do licitante."
        ),
        "name": st.column_config.TextColumn(
            "Nome", required=True, help="Nome do licitante."
        ),
        "website": st.column_config.TextColumn(
            "Website", help="Website do licitante (opcional)."
        ),
        "email": st.column_config.TextColumn(
            "Email", help="Email de contato (opcional)."
        ),
        "phone": st.column_config.TextColumn(
            "Telefone", help="Telefone de contato (opcional)."
        ),
        "desc": st.column_config.TextColumn(
            "Descrição/Observações", help="Outras informações relevantes (opcional)."
        ),
